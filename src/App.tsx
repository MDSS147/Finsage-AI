...
export default function App() {
  ...
  const [sidebarTab, setSidebarTab] = useState<'insights' | 'strategies' | 'calculators' | 'news' | 'admin' | 'portfolio'>('insights');
      ...
      <header className="h-20 glass-header border-b border-white/10 flex items-center justify-between px-6 z-40 buttery-glide">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className={cn(
              "p-2.5 rounded-2xl buttery-glide flex items-center gap-2.5 active:scale-95",
              isLiveActive ? "bg-rose-500/20 text-rose-400 animate-pulse" : "glass-surface text-slate-400 hover:text-brand-primary shadow-sm"
            )}
            title={isLiveActive ? "End Live Conversation" : "Start Live Conversation"}
          >
            <PhoneCall size={20} />
            <span className="text-xs font-bold hidden sm:inline uppercase tracking-widest">{t('live')}</span>
          </button>
          ...
          <button 
            ...
            className={cn(
              "p-2.5 rounded-2xl buttery-glide flex items-center gap-2.5 active:scale-95",
              isVoiceEnabled ? "bg-brand-primary text-white shadow-lg shadow-indigo-500/30" : "glass-surface text-slate-400 hover:text-brand-primary shadow-sm"
            )}
            title={isVoiceEnabled ? "Voice Assistance On" : "Voice Assistance Off"}
          >
            {isVoiceEnabled ? <Volume2 size={20} /> : <VolumeX size={20} />}
            <span className="text-xs font-bold hidden sm:inline uppercase tracking-widest">{t('voice')}</span>
          </button>
      ...
      <main className="flex-1 flex overflow-hidden">
                  ...
                  >
                    {isLiveActive ? (
                      <div className="flex flex-col items-center gap-6">
                        <Waveform isAnimating={isAiSpeaking} levels={audioLevels} barCount={12} className="h-24" />
                        
                        <div className="space-y-1">
                          <p className="text-white/40 font-mono text-[10px] uppercase tracking-[0.3em] text-center">
                            {isAiSpeaking ? 'Model Output' : 'Audio Input'}
                          </p>
                        </div>
                      </div>
                    ) : (
                  ...
                  <div className="flex p-1.5 bg-white/5 rounded-2xl border border-white/10">
                    <button
                      ...
                      className={cn(
                        ...
                        sidebarTab === 'insights' ? "bg-zinc-800 text-brand-primary shadow-sm" : "text-slate-400 hover:text-slate-200"
                      )}
                    >
                      {t('insights')}
                    </button>
                      ...
                      <section>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-4 bg-white/5 rounded-2xl border border-white/5 shadow-sm">
                            <span className="text-[10px] text-slate-500 uppercase font-bold block mb-1">{t('age')}</span>
                            <span className="text-base font-bold text-white">{profile.age} {t('years')}</span>
                          </div>
                          <div className="p-4 bg-white/5 rounded-2xl border border-white/5 shadow-sm">
                            <span className="text-[10px] text-slate-500 uppercase font-bold block mb-1">{t('income')}</span>
                            <span className="text-base font-bold text-white">{formatCurrency(profile.monthlyIncome)}/{t('mo')}</span>
                          </div>
                        </div>
                      </section>
                      <FinancialCharts profile={profile} currency={selectedCurrency} />
                    </>
        ...
        <aside className="hidden lg:flex w-96 glass-sidebar border-r border-white/10 flex-col overflow-y-auto custom-scrollbar">
          <div className="p-8 space-y-10">
              ...
              <div className="flex p-1.5 bg-white/5 rounded-2xl border border-white/10 mb-8 overflow-x-auto custom-scrollbar no-scrollbar gap-1">
                <button
                  ...
                  className={cn(
                    ...
                    sidebarTab === 'insights' ? "bg-zinc-800 text-brand-primary shadow-md" : "text-slate-400 hover:text-slate-200"
                  )}
                >
                  {t('insights')}
                </button>
                ...
                <button
                  ...
                  className={cn(
                    "flex-1 min-w-[80px] py-3 rounded-xl text-[10px] font-bold transition-all uppercase tracking-widest whitespace-nowrap",
                    sidebarTab === 'portfolio' ? "bg-zinc-800 text-brand-primary shadow-md" : "text-slate-400 hover:text-slate-200"
                  )}
                >
                  {t('portfolio')}
                </button>
                <button
                  ...
                  className={cn(
                    "flex-1 min-w-[70px] py-3 rounded-xl text-[10px] font-bold transition-all uppercase tracking-widest whitespace-nowrap",
                    sidebarTab === 'admin' ? "bg-zinc-800 text-brand-primary shadow-md" : "text-slate-400 hover:text-slate-200"
                  )}
                >
                  {t('admin')}
                </button>
              ...
              {sidebarTab === 'insights' ? (
                <div className="space-y-6">
                  <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <TrendingUp size={14} /> {t('marketInsights')}
                  </h3>
                ...
                <div className="space-y-6">
                  <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <ShieldCheck size={14} /> {t('adminSettings')}
                  </h3>
                  <div className="glass-card p-6 rounded-[2rem] border border-white/10 shadow-xl space-y-4">
                    {!isAuthorized ? (
                      <div className="space-y-3">
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{t('enterAuthCode')}</p>
                        <input 
                          type="password"
                          value={authCode}
                          onChange={(e) => setAuthCode(e.target.value)}
                          placeholder="••••"
                          className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-brand-primary transition-all"
                        />
                        <button 
                          onClick={() => {
                            if (authCode === '1234') {
                          ...
                          className="w-full py-3 bg-brand-primary text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-brand-primary/90 transition-all"
                        >
                          {t('verifyCode')}
                        </button>
                      </div>
                      ...
                      <div className="space-y-3">
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{t('changeAppName')}</p>
                        <input 
                          type="text"
                          value={newAppName}
                        ...
                        <button 
                          ...
                          className="w-full py-3 bg-emerald-500 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-emerald-600 transition-all"
                        >
                          Update Name
                        </button>
                      </div>
            ...
            <div className="p-6 bg-brand-primary/10 rounded-[2rem] border border-brand-primary/20 relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-24 h-24 bg-brand-primary/20 rounded-full blur-2xl -mr-12 -mt-12 transition-transform group-hover:scale-150 duration-700" />
              <div className="flex gap-4 mb-3 relative z-10">
                <div className="p-2.5 bg-brand-primary/20 rounded-xl text-brand-primary shadow-inner">
                  <Info size={18} />
                </div>
                <h4 className="text-sm font-bold text-white">Advisor Tip</h4>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed relative z-10 font-medium">
                Consider diversifying into Sovereign Gold Bonds (SGB) for tax-free capital gains if held till maturity.
              </p>
            </div>
          </div>
        </aside>
            ...
            <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto relative group">
              ...
              <div className="absolute right-3 top-3 bottom-3 flex gap-2.5 z-20">
                ...
                >
                  {isListening ? (
                    <>
                      <Waveform isAnimating={true} levels={new Array(4).fill(0).map(() => 0.4 + Math.random() * 0.6)} barCount={4} className="h-6" />
